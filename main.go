...
func main() {
	...
	// 打印配置信息用于调试（不含敏感信息）
	log.Printf("Config loaded: Database Host=%s Port=%d DBName=%s", cfg.Database.Host, cfg.Database.Port, cfg.Database.DBName)
	...
	api := r.Group("/api/v1")
	{
		...
		// 推送任务相关路由
		pushTasks := api.Group("/push-tasks")
		{
			pushTasks.POST("", pushTaskHandler.CreateTask)
			pushTasks.GET("", pushTaskHandler.ListTasks)
			pushTasks.PUT("/:taskId", pushTaskHandler.UpdateTask)
			pushTasks.GET("/:taskId", pushTaskHandler.GetTask)
			pushTasks.DELETE("/:taskId", pushTaskHandler.DeleteTask)
			pushTasks.POST("/estimate", pushTaskHandler.EstimateUsers)
